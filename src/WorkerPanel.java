    ...
    public static void start(String loginDATABASE, String hasloDATABASE, String nameDATABASE, String teamNameDATABASE) {
        while (true) {
            ...
            try {
                ...
                ResultSet resultSelect = QueryExecutor.executeSelect("SELECT pracownicy.login, pracownicy.haslo, pracownicy.imie_pracownika, pracownicy.nazwisko_pracownika, pracownicy.numer_telefonu, pracownicy.stanowisko, ekipy.nazwa FROM `pracownicy` INNER JOIN `ekipy` ON pracownicy.id_zespolu = ekipy.id_ekipy WHERE id_zespolu = ( SELECT id_zespolu FROM pracownicy WHERE login = '"+loginDATABASE+"' ) AND pracownicy.stanowisko = 'kierownik'");

                while (resultSelect.next()) {

                    String teamLeaderNameDATABASE = resultSelect.getString("imie_pracownika");
                    String teamLeaderSurnameDATABASE = resultSelect.getString("nazwisko_pracownika");
                    String teamLeaderContactDATABASE = resultSelect.getString("numer_telefonu");

                    System.out.println("Twój kierownik: " + teamLeaderNameDATABASE + " " + teamLeaderSurnameDATABASE + " - tel: " + teamLeaderContactDATABASE);
